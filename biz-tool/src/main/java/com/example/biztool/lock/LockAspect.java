...
//            log.info("LockAspect lock success is {}, lock key:{},expireSeconds:{}", Objects.nonNull(redisLock), redisKey, expireSeconds);
            AssertUtil.isTrue(LockUtil.lock(redisKey), ErrorCode.REQUEST_FREQUENTLY);
            return joinPoint.proceed();
        } catch (BizException bizException) {
            throw bizException;
        } catch (Exception e) {
//            log.warn("LockAspect lock error key:{},expireSeconds:{}", redisKey, expireSeconds, e);
           AssertUtil.throwException(ErrorCode.DEFAULT_ERROR);
        } finally {
            LockUtil.unlock(redisKey);
        }
        return null;
    }
     ...
     * @return
     */
    private String getRedisKey(Object[] args,String redisKey) {
        //获取key中变量对应的值的键值对
        Map<String, String> fieldName2Value = getFieldName2ValueMap(args, redisKey);
     ...
     * @return
     */
    private Map<String, String> getFieldName2ValueMap(Object[] args, String key) {
        //获取key中变量
